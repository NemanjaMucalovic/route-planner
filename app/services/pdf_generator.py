...
class PDFGenerator:
    ...
    def generate_pdf(self, set_id, image_type):
        ...
        c.drawImage(image_path, (A4[0] - img_width * image_scale) / 2,
                    A4[1] - title_height - img_height * image_scale - image_top_spacing, width=img_width * image_scale,
        ...
        c.drawString((A4[0] - places_header_width) / 2,
                     A4[1] - title_height - img_height * image_scale - places_header_height, places_header)
        y_position = A4[
                         1] - title_height - img_height * image_scale - places_header_height - 20  # Starting position for details
        ...
        for place in places:
            ...
            if y_position - background_height < 50:  # Start new page if content doesn't fit
                c.showPage()
                y_position = A4[1] - background_height - 20  # Reset starting position for details on the next page

            # Draw pale grey background for each place
            c.setFillColor(colors.lightgrey)
            c.rect(50, y_position - background_height, A4[0] - 100, background_height, fill=True)
            c.setFillColor(colors.black)

            c.setFont("Helvetica", 10)
            text_object = c.beginText(70, y_position - background_height + details_line_height + 26)
    ...
    def generate_route_image(set_id,image_type):
        image_path = os.path.join("csv", f"route_image_{set_id}.png")
        if image_type == "directions":
           data = get_specific_field_by_foreign_key(foreign_key=set_id, field_name="overview_polyline",
           ...
           response = requests.get(
            ...
            response = requests.get(f"https://maps.googleapis.com/maps/api/staticmap?size=640x480&{markers_query}&key={os.environ.get('GOOGLE_MAPS_API_KEY')}")
